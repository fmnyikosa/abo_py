...
def manager( model_ , optimizer_ , data_, n_epochs = 1 ):
    ...
    number_burn_in               =  4
    initial_etas                 =  [  1*1e-2 , 5*1e-3 , 1*1e-3 ,  5*1e-4  ] # for initial data

    T, X, Y_L, Y_G               =  [], [], [], []   # data stores for BO
    proposal                     =  [ 0 , 1*1e-3 ]   # placeholder for BO proposal
    ...
    for epoch in range(n_epochs):
        ...
        print 'Associated average loss : ' + str( current_average_loss )
        print '------------------------------------------------'
        print ' '
        ...
        if epoch == (number_burn_in-1):
            ...
            T_          = np.array( T )[ : , None ]
            X_          = np.array( X )[ : , None ]
            Y_L_        = np.array(Y_L)[ : , None ]
            Y_G_        = np.array(Y_G)[ : , None ]
            ...
            # gp_model    = abo.get_preloaded_model(model_, optimizer_)
            # gp_model    = abo.update_bayes_opt_model( gp_model, x , y  )

            gp_model    = abo.initialize_bayes_opt_model_with_der( X__ , Y_L_, Y_G_, 
                                                            n_epochs, first_loss  )

            # set bounds
            lb          = [ epoch + 1.00   ,  7*1.0e-4  ] # (1.0 * 1e-7)
            ub          = [ epoch + 1.00   ,  3*1.0e-2  ] # (9.0 * 1e-1)

            # sample from it
            # proposal    = abo.get_bayes_opt_proposal_lcb( gp_model, iterations, lb, ub   ) # iterations
            proposal    = abo.get_bayes_opt_proposal_lcb_with_der( gp_model, iterations, lb, ub  )

        # c) use model normally - update and sample
        if epoch > (number_burn_in - 1):

            # # prepare new data without derivatives
            # x_new       = [ epoch + 1 , last_eta ]
            # y_new       = current_average_loss
            # y_g         = current_average_grad
            # # update model
            # gp_model  = abo.update_bayes_opt_model( gp_model, x_new, y_new )

            # prepare new data with derivatives
            first_loss  = loss_epochs[   0        ]
            T_          = np.array( T )[ : , None ]
            X_          = np.array( X )[ : , None ]
            Y_L_        = np.array(Y_L)[ : , None ]
            Y_G_        = np.array(Y_G)[ : , None ]
            X__         = np.hstack( ( T_ , X_ ) )
            # update model with derivatives
            gp_model    = abo.initialize_bayes_opt_model_with_der( 
                               X__ , Y_L_ , Y_G_ , n_epochs, first_loss  )
    ...
    mat_name = './logs/' + 'o-deri-' +  model_ + '-' + optimizer_ + '-' + data_ + '-' + \
